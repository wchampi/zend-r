...
/**
 * ZendR_Sf_Yml_Parser parses YAML strings to convert them to PHP arrays.
 *
 * @package    symfony
 * @subpackage yaml
 * @author     Fabien Potencier <fabien.potencier@symfony-project.com>
 * @version    SVN: $Id: ZendR_Sf_Yml_Parser.class.php 10832 2008-08-13 07:46:08Z fabien $
 */
class ZendR_Sf_Yml_Parser
{
        ...
        while ($this->moveToNextLine()) {
            ...
            if (preg_match('#^\-(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values)) {
                ...
                if (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
                    ...
                    $parser = new ZendR_Sf_Yml_Parser($c);
                    $parser->refs = & $this->refs;
                    $data[] = $parser->parse($this->getNextEmbedBlock());
                } else {
                    if (preg_match('/^([^ ]+)\: +({.*?)$/', $values['value'], $matches)) {
                        $data[] = array($matches[1] => ZendR_Sf_Yml_Inline::load($matches[2]));
                    } else {
                        $data[] = $this->parseValue($values['value']);
                    }
                }
            } else if (preg_match('#^(?P<key>' . ZendR_Sf_Yml_Inline::REGEX_QUOTED_STRING . '|[^ ].*?) *\:(\s+(?P<value>.+?))?\s*$#', $this->currentLine, $values)) {
                $key = ZendR_Sf_Yml_Inline::parseScalar($values['key']);
                ...
                // one liner?
                if (1 == count(explode("\n", rtrim($this->value, "\n")))) {
                    $value = ZendR_Sf_Yml_Inline::load($this->lines[0]);
                    if (is_array($value)) {
                        $first = reset($value);
                        if ('*' === substr($first, 0, 1)) {
                            $data = array();
    ...
    protected function parseValue($value)
    {
        ...
        if (preg_match('/^(?P<separator>\||>)(?P<modifiers>\+|\-|\d+|\+\d+|\-\d+|\d+\+|\d+\-)?(?P<comments> +#.*)?$/', $value, $matches)) {
            ...
            return ZendR_Sf_Yml_Inline::load($value);
        }
    }
